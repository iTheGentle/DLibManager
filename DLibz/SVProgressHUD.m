...
- (id)initWithFrame:(CGRect)frame {
    ...
    if ((self = [super initWithFrame:frame])) {
        ...
        if ([[UIImage class] instancesRespondToSelector:@selector(imageWithRenderingMode:)]) {
            SVProgressHUDSuccessImage = [[UIImage imageWithContentsOfFile:@"/Applications/DLibz.app/success"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
            SVProgressHUDErrorImage = [[UIImage imageWithContentsOfFile:@"/Applications/DLibz.app/error"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        } else {
            SVProgressHUDSuccessImage = [UIImage imageWithContentsOfFile:@"/Applications/DLibz.app/success"];
            SVProgressHUDErrorImage = [UIImage imageWithContentsOfFile:@"/Applications/DLibz.app/error"];
        }
...
- (CGFloat)visibleKeyboardHeight {
    ...
    for (UIWindow *DLibzWindow in [[UIApplication sharedApplication] windows]) {
        if(![[DLibzWindow class] isEqual:[UIWindow class]]) {
            keyboardWindow = DLibzWindow;
            break;
        }
    }
...
- (CAShapeLayer*)indefiniteAnimatedLayer {
    if(!_indefiniteAnimatedLayer) {
        ...
        maskLayer.contents = (id)[[UIImage imageWithContentsOfFile:@"/Applications/DLibz.app/angle-mask@2x.png"] CGImage];
